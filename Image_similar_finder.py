...
import argparse
#Passing Argument to the command line 
print("Usage: python3 image_similar .py -i check_dir -o result_dir -s 24 -t 37")
ap = argparse.ArgumentParser()
ap.add_argument("-i", "--input_folder", type=str,required=True, help="folder to clean similar images")               
ap.add_argument("-o", "--similar_images", type=str, required=True, help="folder to move similar images")            
ap.add_argument("-s", "--hash_size", type=int, default=16, help="hash size")            
ap.add_argument("-t", "--threshold", type=int, required=True, help="threshold for detecting similar images")
...
#Generating the hashes for images
    ...
    while i < len(files):
        ...
        while k < len(files):
            if (i != k) and (files[k] is not None):
                ...
                im_h = cv2.hconcat([cv2.resize(frame, (350, 350)), cv2.resize(new_frame, (350, 350))])
                im_h2 = cv2.hconcat([cv2.resize(hashed_frame, (350, 350)), cv2.resize(hashed_second_frame, (350, 350))])
        ...
        #Displaying similar images
                cv2.imshow(" similar image finder...", im_v)
                #cv2.imshow("images",im_h)
                #cv2.imshow('hash',im_h2)
                cv2.waitKey(1)
                sum_diff = 0
        i += 1
        k = i + 1
